...
class Skill {
    ...
    maintain() {
        if (this.level < c.SKILL_CAP) {
            if (this.score - this.deduction >= this.levelScore) {
                this.deduction += this.levelScore * 1;
                this.level += 1;
    ...
    get levelScore() {
        return Math.ceil(25 * Math.pow(this.level + 1, 1.8) - 2 * this.level + 1);
    }
...
class Entity {
    ...
    refreshBodyAttributes() {
        ...
        this.health.set(
            (((this.settings.healthWithLevel) ? 2 * 1.5^this.skill.level : 0) + this.HEALTH) * this.skill.hlt
        );
        ...
        this.penetration = this.PENETRATION + 1.5 * (this.skill.brst + 0.8 * (this.skill.atk - 1) + 12^this.skill.level);
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            let jackpot2 = Math.ceil(util.getFoodJackpot(this.skill.score) / this.collisionArray.length);
            ...
            this.collisionArray.forEach(instance => {
                ...
                } else if (instance.master.type === 'food') {
                  instance.master.skill.score += jackpot2
            ...
            function incoming(message, socket) {
                ...
                case 't': { // player toggle
                    ...
                    switch (tog) {
                        ...
                        // Kick if it sent us shit. shut up
        ...
        return collision => {
            ...
            if (instance.isGhost) {
                ...
                if (grid.checkIfInHSHG(instance)) {
                    util.warn('[SYSTEM]: Ghost removed.'); grid.removeObject(instance);
                }
                return 0;
            }
            if (!instance.activation.check() && !other.activation.check()) { util.warn('[SYSTEM]: Tried to collide with an inactive instance.'); return 0; }
            // Handle walls
            ...
            if ((instance.type === 'particle' && other.type === 'food') || (other.type === 'particle' && instance.type === 'food')) {
            }
            if ((instance.type === 'particle' && other.type === 'tank') || (other.type === 'particle' && instance.type === 'tank')) {
            }
            if ((instance.type === 'particle' && other.type === 'bullet') || (other.type === 'particle' && instance.type === 'bullet')) {
            }if ((instance.type === 'particle' && other.type === 'crasher') || (other.type === 'particle' && instance.type === 'crasher')) {
            }else
...
var maintainloop = (() => {
    ...
    function placeRoids() {
        function placeRoid(type, entityClass) {
            ...
            do { position = room.randomType(type); 
                x++;
                if (x>200) { util.warn("[SYSTEM]: Could not place some roids."); return 0; }
    ...
    let spawnBosses = (() => {
        ...
        let boss = (() => {
            ...
            return {
                prepareToSpawn: (classArray, number, typeOfLocation = 'norm') => {
                    n = number;
                    bois = classArray;
                    loc = typeOfLocation;
                    i = 0
        ...
        return census => {
            if (timer == 2) {
                util.log('[SYSTEM]: Preparing to spawn...');
                util.log('[SYSTEM]: Almost Prepared...')
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(40, 1)) {
                    case 0: 
                        choice = [[Class.beetle_C, Class.beetle_UC, Class.beetle_R, Class.beetle_E, Class.beetle_LG], 2, 'nest'];
                        break;
                }
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 2;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 5));
            let type = (ran.dice(4)) ? ran.choose([Class.beetle_UC, Class.beetle_R, Class.beetle_E, Class.beetle_LG]) : Class.beetle_C;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
    ...
    let makenpcs = (() => {
        ...
        return () => {
            ...
            // Bots
                ...
                // Remove dead ones
                cel = cel.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                cel.forEach(o => {
                    if (o.skill.level < 200) {
                        o.skill.score += 15000;
                        o.skill.maintain();
                    }
                  if (o.upgrades)
    ...
    // The big food function shit packet
    
    let makefood = (() => {
        let food = [], foodSpawners = [], greenFoodSpawners = [];
        // The two essential functions
        function getFoodClass(level) {
            let a = { };
            switch (ran.chooseChance(100, 75, 6.25, 3.25, 2.25, 1.25, 0.75, 0.325, 0.125, 0.075, 0.05, 0.02, 0.01,
                                     0.5, 0.25, 0.075, 0.05, 0.02, 0.0075, 0.005,
                                     0.25, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002,
                                     0.015, 0.005, 0.001
                                    )) {
                ...
                case 9: a = Class.hendecagon; break;
                case 10: a = Class.dodecagon; break;
                case 11: a = Class.triskaidecagon; break;
                case 12: a = Class.tetradecagon; break;
                case 13: a = Class.radoctagon; break;
                case 14: a = Class.radnonagon; break;
                case 15: a = Class.raddecagon; break;
                case 16: a = Class.radhendecagon; break;
                case 17: a = Class.raddodecagon; break;
                case 18: a = Class.radtriskaidecagon; break;
                case 19: a = Class.radtetradecagon; break;
                case 20: a = Class.rad2octagon; break;
                case 21: a = Class.rad2nonagon; break;
                case 22: a = Class.rad2decagon; break;
                case 23: a = Class.rad2hendecagon; break;
                case 24: a = Class.rad2dodecagon; break;
                case 25: a = Class.rad2triskaidecagon; break;
                case 26: a = Class.rad2tetradecagon; break;
                case 27: a = Class.rad3octagon; break;
                case 28: a = Class.rad3nonagon; break;
                case 29: a = Class.rad3decagon; break;
            ...
            if (scatter != -1 || mitosis || seed) {
                ...
                // Brand new
                    ...
                    if (!dirtyCheck(position, 20)) {
                        o = new Entity(room.randomType("food"));
        ...
        let makeGreenFood = () => {
          ...
          o.define(getGreenPolygons(level))
          o.team = -100;
          o.facing = ran.randomAngle();
          food.push(o);
          return o;
